"""
Version 8: add supervised background tasks to the compacting skills agent.

This module keeps the version 7 workspace agent and adds one long-running task
mechanism:

1. `run_bg_task` starts a subprocess without blocking tool execution and drains
   stdout/stderr through in-memory queues.
2. A background supervisor subgraph checks the process, asks the LLM whether to
   wait, report, or kill it, and loops until the task finishes or is killed.
3. The normal agent graph runs through that supervisor before returning to the
   main LLM, so background-task supervision remains part of the conversation
   state.

It still includes the repo-local skill loader, todo tools, nested agent
invocation, and two-stage conversation compaction:
...
The goal is to preserve continuity while letting the agent launch and supervise
commands that run longer than the normal foreground shell timeout.
"""
